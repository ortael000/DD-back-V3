...
export async function calculateCharacterFull(db: any, id: number): Promise<CharacterFulltype> {
  ...
  const fullCharacteristics : CharacterFulltype['Characteristics']= {
    ...
    const maxMana = 15 + fullCharacteristics.Power.Modifier * 6 + fullCharacteristics.Intelligence.Modifier + fullCharacteristics.Constitution.Modifier*2+ levelInfo.level * 2 + flatBonus.Mana;
